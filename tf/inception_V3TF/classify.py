...
def run_inference_on_image(imagePath, modelpath):
    answer = None

    modelFullPath = modelpath +'/output_graph.pbtxt' # join model directory with the name of the graph .pbtxt file
    labelsFullPath = modelpath + '/output_labels.txt' # join model directory with the name of the labels .txt file
    ...
    with tf.Session() as sess:
        ...
        softmax_tensor = sess.graph.get_tensor_by_name('final_result:0') # modeified last layer name
        #softmax_tensor = sess.graph.get_tensor_by_name('softmax:0') # original last layer name
